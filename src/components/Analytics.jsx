...
const Analytics = () => {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      ...
      console.log('✅ Phase 1 complete - showing data immediately');
      setTrackingLinks(linksData);
      setSummary(summaryData);
      setLoading(false); // Show the main data immediately

      // PHASE 2: Load heavy analytics data in background (slower)
      console.log('🚀 Phase 2: Loading analytics data in background...');
      const [referrerAnalytics, visitsData] = await Promise.all([
  ...
  const handleRegenerateBitlyLink = async (linkId) => {
    try {
      const result = await regenerateBitlyLink(linkId);
      ...
      if (result.success) {
        ...
        setTrackingLinks(prev => 
          prev.map(link => 
            link.id === linkId 
              ? { ...link, trackingUrl: result.shortUrl, bitlySuccess: true, bitlyId: result.id }
              : link
          )
        );
        
        alert('Bit.ly link regenerated successfully!');
      } else {
        alert('Failed to regenerate Bit.ly link. Please try again.');
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search tracking links by name, description, or URL..."
              value={searchTerm}
        ...
        {(() => {
          const filteredLinks = trackingLinks.filter(link => 
            ...
            link.trackingUrl?.toLowerCase().includes(searchTerm.toLowerCase())
          );
          ...
          if (filteredLinks.length === 0) {
            return (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">No matching links found</h3>
                <p className="text-gray-500">
                  Try adjusting your search terms or create a new tracking link
                </p>
              </div>
            );
          }
                  ...
                  <tr key={link.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        {!link.bitlySuccess && (
                          <button
                            onClick={() => handleRegenerateBitlyLink(link.id)}
                            className="text-purple-600 hover:text-purple-800"
                            title="Create Bit.ly short link"
                          >
