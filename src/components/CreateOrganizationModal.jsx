...
import { Building2, MapPin, Loader2, AlertTriangle } from 'lucide-react';
...
const CreateOrganizationModal = ({ isOpen, onClose, onOrganizationCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send Teams notification for organization creation (don't wait for it, run in background)
      try {
        ...
        console.log('📤 Sending organization creation Teams notification with data:', organizationNotificationData);
        ...
        sendOrganizationCreatedNotification(organizationNotificationData)
          .then(success => {
            if (success) {
              ...
              console.log('⚠️ Organization creation Teams notification failed (non-critical)');
            }
          })
          .catch(error => {
            console.error('⚠️ Organization creation Teams notification error (non-critical):', error);
          });
      } catch (error) {
        console.error('⚠️ Organization creation Teams notification setup failed (non-critical):', error);
      }
