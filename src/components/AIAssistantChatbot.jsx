...
const AIAssistantChatbot = () => {
  ...
  useEffect(() => {
    // Check if user has already interacted with chatbot in this session
    const hasInteracted = sessionStorage.getItem('chatbot_interacted');
    
    if (!hasInteracted && !hasAutoOpened && !isOpen) {
      autoOpenTimerRef.current = setTimeout(() => {
        console.log('🤖 Auto-opening AI Assistant after 15 seconds');
        ...
        const welcomeMessage = language === 'de' 
          ? "Hallo! 👋 Ich bin Ihr AI-Assistent. Wie kann ich Ihnen heute helfen?"
        ...
        // Mark that chatbot has been auto-opened for this session
  ...
  }, [hasAutoOpened, isOpen, playNotificationSound, language]);

  // Mark user interaction when they manually open/close the chatbot
  useEffect(() => {
    if (isOpen) {
