...
// import logo from "../images/logo.png" // Moved to public/images
...
export default function RapidWorksHeader() {
  ...
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === 'organizationContextChanged' && currentUser) {
  ...
  const currentPath = pathname.split("/")[1] ? `/${pathname.split("/")[1]}` : "/branding";
      ...
      <div className="w-full px-8">
        ...
        <div className="h-6">
          ...
          <Link href="/" className="flex items-center group">
              <img src="/images/logo.png" alt="RapidWorks" className="h-10 w-auto" />
            </Link>
            <button
                  ...
                  <div key={item.name} className="relative group">
                    <Link href={item.path}
                      className={`relative px-2 lg:px-4 py-2 rounded-full flex items-center gap-1 lg:gap-2 transition-all duration-300 text-xs lg:text-sm font-medium z-10 whitespace-nowrap ${
                        isActive(item.path)
                          ? `text-black`
                          : `text-black hover:text-[${accentColor}]`
                      }`}
                    >
