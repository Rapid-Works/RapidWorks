...
// New function for VB form submission
export const submitVBFormToAirtable = async (formData) => {
  try {
    const submitVBForm = httpsCallable(functions, 'submitVBForm');
    const result = await submitVBForm(formData);
    
    return result.data;
  } catch (error) {
    console.error("Error submitting VB form:", error);
    throw error;
  }
};
